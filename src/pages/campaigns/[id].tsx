...
const CampaignDetail = () => {
  ...
  const handlerModal = (candidateId: number) => {
    const candidate =
      data &&
      data.candidates.find((candidate: any) => candidate.id === candidateId);
  ...
  const fetchData = async () => {
    const qId = typeof query.id === "string" ? parseInt(query.id) : undefined;
        ...
        if (res.data.campaigns?.title) {
          if (res.data.campaigns?.finished) {
            const winner = res.data.campaigns?.candidates?.find(
              (candidate: Candidate) =>
                candidate.number == res.data.campaigns?.winner_candidate
            );
          ...
          if (res.data.campaigns?.allowedEmails?.length != 0) {
            ...
            const eligbible = res.data.campaigns?.allowedEmails?.some(
  ...
  return (
    <>
      <Head>
        <title>{data?.title || "Campaign Title"} | E-Voting</title>
      </Head>
      <div>
        {(isLoading || isVoting) && (
        ...
        <section className="container max-w-6xl mx-auto mt-6 p-2 sm:p-4">
          ...
          <p className="text-sm sm:text-base">
            Berikut adalah daftar kandidat yang berpartisipasi dalam campaign
            ini. Baca visi, misi, dan program kerja mereka sebelum memilih!
          </p>
          {/* CANDIDATES */}
        ...
        <section className="container max-w-6xl mx-auto mt-6 p-2 sm:p-4">
          ...
          {data && data.allowedEmails.length > 0 ? (
            <div className="collapse">
              <label htmlFor="collapseMail" className="hidden"></label>
                ...
                <p>
                  ...
                  {data.allowedEmails?.length <= 2
                    ...
                    : data.allowedEmails
                        ...
                        .join(", ")}
                  {data.allowedEmails?.length > 2 &&
                    ` and ${data.allowedEmails?.length - 2} more`}
              ...
              <div className="collapse-content">
                ...
                <ul className="list-decimal ms-4">
                  {data.allowedEmails?.map((e: AllowedEmail, index: number) => (
                    ...
                    data?.candidates?.map((candidate: any) => (
                      <div key={candidate.id}>
                        <p className="font-semibold text-center text-3xl sm:sm:text-4xl md:text-5xl">
                          {(data &&
                            data.result?.[candidate.id]?.percentage?.toFixed(
                              1
                            )) ||
                            "0"}
                          %
                        ...
                        <div className="flex justify-center gap-2">
                          <p className="text-sm">
                            {`Candidate ${candidate.number} • ${
                              (data &&
                                data.result?.[candidate.id]?.totalVotes) ||
                              "0"
                            } Voters`}
                          </p>
                        </div>
        ...
        >
          <div className="modal-box rounded-lg p-4">
            ...
            <p className="text-sm text-gray-500">
              You&apos;re about to vote for {candidateDetail?.chief_name} and{" "}
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-end">
                <p className="text-right text-sm text-gray-500">No</p>
            ...
            <div className="modal-action mt-3 gap-2">
              <form method="dialog" className="w-full flex justify-end gap-2">
                ...
                <button
                  ...
                  className={`btn ${
                    isVoted ? "btn-disabled cursor-not-allowed" : "btn-primary"
        ...
        >
          <div className="modal-box rounded-lg p-6">
            ...
            <div className="border border-gray-300 p-2 rounded-lg mb-2">
              <p className="text-gray-500 text-sm">{candidateDetail?.visi}</p>
              <ol className="text-gray-500 text-sm list-decimal ms-4">
                <li>
                  Meningkatkan transparansi dan partisipasi anggota dalam setiap
                  kegiatan organisasi.
                </li>
                <li>
                  Mengoptimalkan penggunaan teknologi untuk efisiensi kerja
                  organisasi.
                </li>
                <li>
                  Mengembangkan program kerja yang inovatif dan bermanfaat bagi
                  seluruh anggota.
                </li>
              </ol>
              <p className="text-gray-500 text-sm">
                Visi kami adalah menjadikan organisasi lebih modern, inklusif,
            ...
            <div className="border border-gray-300 p-2 rounded-lg mb-2">
              <p className="text-gray-500 text-sm">{candidateDetail?.misi}</p>
              <ol className="text-gray-500 text-sm list-decimal ms-4">
                <li>
                  Meningkatkan komunikasi antar anggota melalui platform
                  digital.
                </li>
                <li>
                  Menyelenggarakan pelatihan dan workshop rutin untuk
                  pengembangan diri.
                </li>
                <li>Memperkuat kolaborasi dengan organisasi lain.</li>
              </ol>
              <p className="text-gray-500 text-sm">
                Misi kami adalah memberikan kontribusi nyata dan solusi inovatif
            ...
            <div className="border border-gray-300 p-2 rounded-lg">
              ...
              <ol className="text-gray-500 text-sm list-decimal ms-4">
                <li>Program digitalisasi administrasi organisasi.</li>
                <li>Program pelatihan kepemimpinan dan soft skill.</li>
                <li>Program pengabdian masyarakat dan kolaborasi eksternal.</li>
              </ol>
              <p className="text-gray-500 text-sm">
                Setiap program kerja dirancang untuk meningkatkan kualitas dan
                daya saing anggota organisasi.
              </p>
            </div>
