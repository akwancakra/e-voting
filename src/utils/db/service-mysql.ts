    ...
    const campaigns = await prisma.campaign.findMany({
        ...
        where: {
            ...
            OR: [
                ...
                { candidates: { some: { vice_name: { contains: keyword } } } },
                { candidates: { some: { chief_name: { contains: keyword } } } },
            ],
        },
        take: limit,
        skip: skip,
...
export async function getCampaignById({ id = 0 }: { id?: number }) {
    ...
    if (campaign) {
        ...
        campaign.userVoteCampaign.forEach((vote) => {
